...
if st.session_state.running and not st.session_state.done:
    ...
    with st.spinner("📄  Reader Agent is scraping top resources…"):
        ...
        rr = reader_agent.invoke({
            "messages": [("user",
                f"You are a web scraping agent. You MUST call the scrape_url tool with a real URL.\n\n"
                f"Here are search results about '{topic_val}':\n\n"
                f"{results['search']}\n\n"
                f"Instructions:\n"
                f"1. Pick the single most relevant URL from the search results above.\n"
                f"2. Call the scrape_url tool with that exact URL (do NOT invent or placeholder URLs).\n"
                f"3. Return the scraped content you receive from the tool."
            )]
        })
